...
LOCAL_CHART_PATH="helm/sbom-service-chart"
...
echo "--- Building local sbom-service image inside of Minikube ---"
# Will have local image sbom-service:latest in minikube
bash ./hack/build-local-sbom-service-into-minikube.sh
...
target_name = 'sbom-service-chart'
...
for line in lines:
    ...
    # If we are inside the sbom-service-chart block, replace values
    if in_block:
...
kubectl rollout restart deployment -n $NAMESPACE -l app.kubernetes.io/name=sbom-service-chart || true
